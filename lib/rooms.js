...
const DEFAULTS = {
  ...
  emptyMs: 5 * 60 * 1000,      // 最後一人離線後多久回收（包含斷線保留期間）
  turnMs: Rules.CONST.TURN_MS, // 每回合出手時限
...
class Room {
  constructor(store, opts) {
    ...
    this.version = 0;
    this.turnDeadline = 0;
    this.selectedItems = { cat: null, dog: null }; // 本回合已鎖定的道具
  ...
  start(id, now) {
    ...
    this.summary = [];
    this.lastShot = null;
    this.selectedItems = { cat: null, dog: null };
  ...
  /** 選定本回合道具；第一次選定後直到出手／跳過前不能更換。 */
  selectItem(id, item, now) {
    if (this.phase !== 'playing' || !this.state) return err('現在沒有進行中的對局。');
    const m = this.member(id);
    if (!m) return err('你不在這個房間裡。');
    const side = this.sideOf(id);
    if (!side) return err('觀戰者不能選擇道具，只有場上的玩家可以。', 'forbidden');
    if (this.ai[side]) return err('這一邊由電腦控制。', 'forbidden');
    if (this.state.turn !== side) return err('現在還沒輪到你，不能選道具。');
    if (this.turnDeadline && now >= this.turnDeadline) return err('出手時間已到，這一回合會自動跳過。', 'timeout');

    const key = String(item || '');
    const it = Rules.ITEMS[key];
    if (!it || it.kind !== 'shot') return err('只能選擇發射型道具。');
    if (!Rules.hasItem(this.state, side, key)) return err(it.label + '已經用完了。');

    const selected = this.selectedItems[side];
    if (selected) {
      if (selected === key) return ok({ item: key, already: true });
      return err('本回合已選定' + Rules.ITEMS[selected].label + '，不能更換道具。', 'item_locked');
    }

    this.selectedItems[side] = key;
    this.touch();
    return ok({ item: key });
  }
  ...
  fire(id, action, now) {
    ...
    if (this.turnDeadline && now >= this.turnDeadline) {
      return err('出手時間已到，這一回合會自動跳過。', 'timeout');
    }
    const requestedItem = action && action.item ? String(action.item) : null;
    const selected = this.selectedItems[side];
    if (selected && requestedItem !== selected) {
      return err('本回合已選定' + Rules.ITEMS[selected].label + '，不能更換道具。', 'item_locked');
    }
  ...
  heal(id, now) {
    ...
    if (this.selectedItems[side]) {
      return err('本回合已選定' + Rules.ITEMS[this.selectedItems[side]].label + '，不能改用補血。', 'item_locked');
    }
  ...
  timeoutTurn(now) {
    ...
    const result = Rules.applyPass(this.state, side, '出手時間到，這一回合跳過。');
    if (!result.ok) return err(result.reason);
  ...
  turnExpired(now) {
    return this.phase === 'playing' && !!this.state && !!this.turnDeadline && now >= this.turnDeadline;
  }
  ...
  commitShot(result, now, extra) {
    ...
    this.selectedItems[result.shot.side] = null;
    const entry = Object.assign({
      ...
      hitParts: result.shot.hitParts || { cat: null, dog: null },
      hitPart: result.shot.hitPart || null,
      critical: !!result.shot.critical,
  ...
  leave(id, now) {
    ...
    /* 若斷線時已開始空房倒數，清掉保留座位不能讓倒數重新起算。 */
    if (this.members.size === 0 && !this.emptySince) this.emptySince = now;
    this.touch();
  ...
  viewFor(id, now) {
    ...
    return {
      ...
      you: m ? {
        ...
        ready: m.ready,
        isHost,
        selectedItem: side ? (this.selectedItems[side] || null) : null,
