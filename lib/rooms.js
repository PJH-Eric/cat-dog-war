...
class Room {
  ...
  fireAi(side, now, rng) {
    ...
    const shot = AI.chooseShot(this.state, side, this.ai[side], rng);
    const result = Rules.applyShot(this.state, side, shot);
    if (!result.ok) return err(result.reason);
    this.commitShot(result, now, { aiLevel: this.ai[side], sims: shot.sims });
    return ok({ shot: result.shot, state: this.state });
  }
  ...
  leave(id, now) {
    ...
    if (side) {
      ...
      if (this.phase === 'playing' && this.state && !this.state.over) {
        ...
        this.state.winner = Rules.other(side);
        this.state.reason = m.name + '離開了房間，' + Rules.SIDE_LABEL[this.state.winner] + '不戰而勝。';
        this.state.version += 1;
        this.phase = 'finished';
        this.turnDeadline = 0;
        forfeited = this.state.winner;
      }
    }
